...
class LoginPage(BasePage):
    ...
    def should_be_login_form(self):
        assert self.is_element_present(*LoginPageLocators.LOGIN_FORM), \
            LOGGER.error(f'There is no login form on {self.url} page.')

    def should_be_register_form(self):
        assert self.is_element_present(
            *LoginPageLocators.REGISTRATION_FORM), \
            LOGGER.error(f'There is no registration form on {self.url} page.')
    ...
    def should_be_field_email_address_in_register_form(self):
        assert self.is_element_present(
            *LoginPageLocators.REGISTRATION_EMAIL), \
            LOGGER.error(f'There is no Email address field on {self.url} page.')

    def should_be_field_password_in_register_form(self):
        assert self.is_element_present(
            *LoginPageLocators.REGISTRATION_PASWD), \
            LOGGER.error(f'There is no Password field on {self.url} page.')

    def should_be_field_confirm_password_in_register_form(self):
        assert self.is_element_present(
            *LoginPageLocators.REGISTRATION_CONFIRM_PASWD), \
            LOGGER.error(f'There is no Confirm password field on {self.url} page.')

    def should_be_button_register_in_register_form(self):
        assert self.is_element_present(
            *LoginPageLocators.BUTTON_REGISTER), \
            LOGGER.error(f'There is no Register button on {self.url} page.')
