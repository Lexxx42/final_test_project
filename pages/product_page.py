...
class ProductPage(BasePage):
    def should_be_add_to_basket_button(self):
        assert self.is_element_present(
            *ProductPageLocators.BUTTON_ADD_TO_BASKET), \
            LOGGER.error(f'No Add to basket button on {self.url} page')
    ...
    def should_be_message_of_added_product(self):
        assert self.is_element_present(
            *ProductPageLocators.MESSAGE_WITH_PRODUCT_NAME), \
            LOGGER.error(f'No message of added product on {self.url} page')
    ...
    def should_be_product_name_message_match_product_name_card(self):
        ...
        assert product_name_in_message.text == product_name_in_card.text, \
            LOGGER.error('Product name added to the basket not match product name on the product card.' \
    ...
    def should_be_message_of_basket_total(self):
        assert self.is_element_present(
            *ProductPageLocators.MESSAGE_OF_BASKET_COST), \
            LOGGER.error(f'No message of basket total on {self.url} page')
    ...
    def should_be_equal_basket_price_in_message_and_product_prise(self):
        ...
        assert basket_price_in_message.text == product_price_in_card.text, \
            LOGGER.error(f'Product cost on the card should be equal basket cost. '
